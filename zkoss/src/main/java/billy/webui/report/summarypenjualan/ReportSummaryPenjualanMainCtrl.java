...
public class ReportSummaryPenjualanMainCtrl extends GFCBaseCtrl implements Serializable {
	...
	public void onClick$btnViewPdf(Event event) throws Exception {
		if(validToPrint()){
			final Window win = (Window) Path.getComponent("/outerIndexWindow");
			new ViewPdfLaporanSummaryPenjualanDJReport(win,karyawan,txtb_tanggalAwalPenjualan.getValue(),txtb_tanggalAkhirPenjualan.getValue(),listPenjualan,selectedPrinter);
		}else{
			showErrorCetak();			
		}	
	}
