...
public class CetakLaporanSummaryPenjualanDJReport extends Window implements Serializable {
  // TIDAK TERPAKAI HANYA CONTOH PAKAI TEMPLATE
  private static final long serialVersionUID = 1L;
  private static final Logger logger = Logger.getLogger(CetakLaporanSummaryPenjualanDJReport.class);

  public CetakLaporanSummaryPenjualanDJReport(Component parent, Karyawan karyawan, Date startDate,
  ...
  public void doPrint(Karyawan karyawan, Date startDate, Date endDate,
      List<Penjualan> listPenjualan, PrintService selectedPrinter) {
    SimpleEscp simpleEscp = new SimpleEscp(selectedPrinter.getName());
    Template template = null;
    try {
      template =
          new JsonTemplate(
              CetakLaporanSummaryPenjualanDJReport.class.getResourceAsStream("fakturTemplate.json"));
