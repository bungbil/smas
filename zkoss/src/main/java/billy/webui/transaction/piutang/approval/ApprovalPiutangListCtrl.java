...
public class ApprovalPiutangListCtrl extends GFCBaseListCtrl<Piutang> implements Serializable {

  private static final long serialVersionUID = -2170565288232491362L;
  private static final Logger logger = Logger.getLogger(ApprovalPiutangListCtrl.class);

  protected Window windowApprovalPiutangList; // autowired
  protected Panel panelApprovalPiutangList; // autowired

  protected Borderlayout borderLayout_approvalPiutangList; // autowired
  protected Paging paging_ApprovalPiutangList; // autowired
  protected Listbox listBoxApprovalPiutang; // autowired
   ...
   */
  public ApprovalPiutangListCtrl() {
    super();
  }
  ...
  public void doAfterCompose(Component window) throws Exception {
    ...
    if (arg.containsKey("ModuleMainController")) {
      ...
      getApprovalPiutangMainCtrl().setApprovalPiutangListCtrl(this);
  ...
  public void doFillListbox() {
    ...
    paging_ApprovalPiutangList.setPageSize(getCountRows());
    paging_ApprovalPiutangList.setDetailed(true);
    ...
    getPagedBindingListWrapper().init(searchObj, getListBoxApprovalPiutang(),
        paging_ApprovalPiutangList);
    BindingListModelList lml = (BindingListModelList) getListBoxApprovalPiutang().getModel();
    setPiutangs(lml);
    ...
    if (getSelectedPiutang() == null) {
      ...
      if (lml.getSize() > 0) {
        ...
        // Event
        // yet.
        getListBoxApprovalPiutang().setSelectedIndex(rowIndex);
        // get the first entry and cast them to the needed object
        setSelectedPiutang((Piutang) lml.get(0));

        // call the onSelect Event for showing the objects data in the
        // statusBar
        Events.sendEvent(new Event("onSelect", getListBoxApprovalPiutang(), getSelectedPiutang()));
      }
    }
  ...
  public void doFitSize() {
    ...
    borderLayout_approvalPiutangList.setHeight(String.valueOf(maxListBoxHeight) + "px");

    windowApprovalPiutangList.invalidate();
  }
  ...
  public Listbox getListBoxApprovalPiutang() {
    return this.listBoxApprovalPiutang;
  }
  ...
  public void onCreate$windowApprovalPiutangList(Event event) throws Exception {
  ...
  public void onDoubleClickedPiutangItem(Event event) {
    ...
    if (anPiutang != null) {
      ...
      if (getApprovalPiutangMainCtrl().getApprovalPiutangDetailCtrl() == null) {
        Events.sendEvent(new Event("onSelect",
            getApprovalPiutangMainCtrl().tabApprovalPiutangDetail, null));
        // if we work with spring beanCreation than we must check a
        // little bit deeper, because the Controller are preCreated ?
      } else if (getApprovalPiutangMainCtrl().getApprovalPiutangDetailCtrl().getBinder() == null) {
        Events.sendEvent(new Event("onSelect",
            getApprovalPiutangMainCtrl().tabApprovalPiutangDetail, null));
      }

      Events.sendEvent(new Event("onSelect", getApprovalPiutangMainCtrl().tabApprovalPiutangDetail,
          anPiutang));
    }
  }
   ...
   * @param event
   */
  public void onSelect$listBoxApprovalPiutang(Event event) {
    // logger.debug(event.toString());
    ...
    getApprovalPiutangMainCtrl().getApprovalPiutangDetailCtrl().setSelectedPiutang(anPiutang);
    getApprovalPiutangMainCtrl().getApprovalPiutangDetailCtrl().setPiutang(anPiutang);

    // store the selected bean values as current
    getApprovalPiutangMainCtrl().doStoreInitValues();
  ...
  public void setListBoxApprovalPiutang(Listbox listBoxApprovalPiutang) {
    this.listBoxApprovalPiutang = listBoxApprovalPiutang;
  }
