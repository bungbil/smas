...
public class BarangMainCtrl extends GFCBaseCtrl implements Serializable {
  ...
  public void doAfterCompose(Component window) throws Exception {
     ...
     * 2. Set the barang 'recurse' to 'false' to avoid problems with managing more than one zul-file
     * in one page. Otherwise it would be overridden and can ends in curious error messages.
     */
  ...
  private void doSave(Event event) throws InterruptedException {
    ...
    try {
      ...
      barangCheckKode =
          getBarangService().getBarangByKodeBarang(
              getBarangDetailCtrl().getBarang().getKodeBarang());

      if (barangCheckKode != null) {
        if (barangCheckKode.getId() != getBarangDetailCtrl().getBarang().getId()) {
          ZksampleMessageUtils.showErrorMessage("Kode Barang sudah digunakan oleh barang : "
              + barangCheckKode.getNamaBarang());
          return;
        }
      }
  ...
  private void doSkip(Event event) {
    ...
    BindingListModelList blml =
        (BindingListModelList) getBarangListCtrl().getListBoxBarang().getModel();
    ...
    Events.sendEvent(new Event(Events.ON_SELECT, getBarangListCtrl().getListBoxBarang(),
        getSelectedBarang()));
  ...
  public void onCheck$checkbox_BarangList_ShowAll(Event event) {
    ...
    HibernateSearchObject<Barang> soBarang =
        new HibernateSearchObject<Barang>(Barang.class, getBarangListCtrl().getCountRows());
  ...
  public void onClick$button_BarangList_SearchName(Event event) throws Exception {
    ...
    if (!txtb_Barang_Name.getValue().isEmpty()) {
      ...
      HibernateSearchObject<Barang> soBarang =
          new HibernateSearchObject<Barang>(Barang.class, getBarangListCtrl().getCountRows());
      soBarang.addFilter(new Filter("namaBarang", "%" + txtb_Barang_Name.getValue() + "%",
          Filter.OP_ILIKE));
      soBarang.addSort("namaBarang", false);
  ...
  public void onCreate$windowBarangMain(Event event) throws Exception {
    ...
    if (tabPanelBarangList != null) {
      ZksampleCommonUtils.createTabPanelContent(this.tabPanelBarangList, this,
          "ModuleMainController", "/WEB-INF/pages/master/barang/barangList.zul");
    }
  ...
  public void onSelect$tabBarangDetail(Event event) throws IOException {
    ...
    if (tabPanelBarangDetail != null) {
      ZksampleCommonUtils.createTabPanelContent(this.tabPanelBarangDetail, this,
          "ModuleMainController", "/WEB-INF/pages/master/barang/barangDetail.zul");
    }
  }
