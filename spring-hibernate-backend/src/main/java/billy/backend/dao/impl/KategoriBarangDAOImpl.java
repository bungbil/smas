...
@Repository
public class KategoriBarangDAOImpl extends BillyBasisDAO<KategoriBarang> implements
    KategoriBarangDAO {
  ...
  public int getCountAllKategoriBarangs() {
    return DataAccessUtils.intResult(getHibernateTemplate().find(
        "select count(*) from KategoriBarang"));
  }
  ...
  public KategoriBarang getKategoriBarangByKodeKategoriBarang(String string) {
    ...
    return (KategoriBarang) DataAccessUtils.uniqueResult(getHibernateTemplate().findByCriteria(
        criteria));
  }
